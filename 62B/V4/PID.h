...
long setDistance = 0;
float setGyro = 0.0;
float kKalmanTimer = 0.0;
const tSensors kKalmanSensor;
...
void kalmanInit(const float kSensor, const float kTime)
{
  kKalmanTimer = kTime;
  kKalmanSensor = kSensor;
}
...
task gyroPID() {
  ...
  SimpleKalmanInit(&filter1, 0.025);

  float gyroOutK = SimpleKalmanUpdate(&filter1, SensorValue[s_gyro]);

  while(true) {
    error = setGyro - gyroOutK;
