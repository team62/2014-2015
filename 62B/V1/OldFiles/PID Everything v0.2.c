...
// lift
const ubyte kNumLiftEncodersPerSide = 2;
const tSensors kLiftEncoderPort[kNumLiftEncodersPerSide*2] = {port5, port6, port7, port8};
...
long setDistance = 0;
float setGyro = 0.0;
word setLift = nMotorEncoder[kLiftEncoderPort[0]];
...
task syncPID() {
  ...
  while(true) {
    avgEncoderLeft = 0.0;
    for(ubyte j=0; j<kNumLiftEncodersPerSide*2; j++) {
      if(nMotorDriveSide[kLiftEncoderPort[j]] == driveLeft)
        avgEncoderLeft += nMotorEncoder[kLiftEncoderPort[j]];
      else
        avgEncoderRight += nMotorEncoder[kLiftEncoderPort[j]];
    }
    avgEncoderLeft /= kNumLiftEncodersPerSide;
    avgEncoderRight /= kNumLiftEncodersPerSide;
...
task liftPID() {
  ...
  while(true) {
    ...
    avgEncoder = 0.0;
    for(ubyte j=0; j<kNumLiftEncodersPerSide*2; j++)
      avgEncoder += nMotorEncoder[kLiftEncoderPort[j]];
    avgEncoder /= kNumLiftEncodersPerSide*2;
