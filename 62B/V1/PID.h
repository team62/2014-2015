...
const tSensors kDriveEncoderPort[kNumDriveEncoders] = {m_BL, m_BR};
const ubyte kNumDriveMotors = 4;
const tMotor kDriveMotorPort[kNumDriveMotors] = {m_FL, m_BL, m_FR, m_BR};
const float kWheelDiameter = 2.75; // inches
const bool kDriveIME = true; // set to false if using quadrature encoders
// lift
const ubyte kNumLiftPotsPerSide = 1;
const tSensors kLiftPot[kNumLiftPotsPerSide*2] = {m_LTL, m_RTL};
const ubyte kNumLiftMotors = 4;
const tMotor kLiftMotorPort[kNumLiftMotors] = {m_LTL, m_LTR, m_RTL, m_RTR};
const ubyte kNumLiftValues = 2;
const word kLiftValues[kNumLiftValues] = {0, 4500}; // sort in order from low to high
...
long setDistance = 0;
float setGyro = 0.0;
word setLift = SensorValue[kLiftPot[0]];
...
task syncPID() {
  ...
  const float kD = 0.2;
  const float kL = 50.0;
  const float kPotDiff = 0.0; // diff = left - right

  float avgEncoderLeft = 0.0;
  float avgEncoderRight = 0.0;
  float error = 0.0;
  float prevError = 0.0;
  float p = 0.0;
  float i = 0.0;
  ...
  while(true) {
    avgEncoderLeft = 0.0;
    for(ubyte j=0; j<kNumLiftPotsPerSide*2; j++) {
      if(nMotorDriveSide[kLiftPot[j]] == driveLeft)
        avgEncoderLeft += nMotorEncoder[kLiftPot[j]];
      else
        avgEncoderRight += nMotorEncoder[kLiftPot[j]];
    }
    avgEncoderLeft /= kNumLiftPotsPerSide;
    avgEncoderRight /= kNumLiftPotsPerSide;

    error = avgEncoderLeft - avgEncoderRight - kPotDiff;

    p = error;
    i = limit(i + error, kL);
    d = error - prevError;

    syncSpeed = (word)(p*kP + i*kI + d*kD);
...
task liftPID() {
  ...
  while(true) {
    ...
    avgEncoder = 0.0;
    for(ubyte j=0; j<kNumLiftPotsPerSide*2; j++)
      avgEncoder += SensorValue[kLiftPot[j]];
    avgEncoder /= kNumLiftPotsPerSide*2;

    if(!bIfiAutonomousMode && (joyValue >= kJoyThresh && avgEncoder <= kLiftValues[kUpLim] || joyValue <= -kJoyThresh && avgEncoder >= kLiftValues[kDownLim])) {
      setLift = avgEncoder;
      liftSpeed = joyValue + (127 - abs(joyValue))/(127 - kJoyThresh)*kHoldPower;
    }
    else {
      if(avgEncoder < kLiftValues[kDownLim] + 100) {
        holdDir = -1;
      ...
      i = limit(i + error, kL);
      d = error - prevError;
      liftSpeed = (word)(p*kP + i*kI + d*kD) + holdDir*kHoldPower;
    ...
    // set drive motors' powers based on lift side
